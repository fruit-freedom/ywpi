...
def is_ywpi_object(data: any):
    return isinstance(data, dict) \
        and ('tp' in data and isinstance(data['tp'], str)) \
        and ('data' in data and isinstance(data['data'], dict))


async def create_objects_from_event_data(data: models.TaskUpdatedData):
    task: dict = await tasks_collection.find_one({ '_id': data.id }, { 'borrowed_fields': 1, 'method': 1 })
    ...
    if data.outputs is not None:
        for key, value in data.outputs.items():
            try:
                if isinstance(value, str):
                    ...
                    for relname, relvalue in task.get('borrowed_fields', {}).items():
                        ...
                        relations.append(Relation(
                            object_id=relvalue['object_id'],
                            name=relvalue['path'],
                            source_task=Relation.SourceTask(id=str(task['_id']), method=task['method'])
                        ))
                ...
                traceback.print_exc()
            try:
                if is_ywpi_object(value):
                    print('Detected ywpi object in outputs with type', value['tp'])
                    created_object = await create_object(
                        project_id='67b0a2454e82f0a2172ebb47',
                        tp=value['tp'],
                        data=value['data'],
                        labels=[Label(name=l['name'], value=l.get('value', None)) for l in value.get('labels', [])],
...
async def update_outputs_object(data: models.TaskUpdatedData):
    try:
        task: dict = await tasks_collection.find_one({ '_id': data.id }, { 'context_id': 1 })

        context_id = task['context_id']
        outputs = data.outputs

        print('context_id', context_id, outputs)

        if isinstance(outputs, dict) and '__others__' in outputs:
            objects_collection.update_one({ '_id': ObjectId(context_id) }, {
                '$push': {
                    'data.__others__': {
                        '$each': outputs['__others__']
                    }
                }
            })
    except:
