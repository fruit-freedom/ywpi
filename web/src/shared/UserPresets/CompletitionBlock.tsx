...
interface CompletitionBlockProps {
    agents: Agent[];
    onChange?: (method?: MethodWithAgent) => void;
    activeMethod?: MethodWithAgent;
}

export default ({ agents, activeMethod, onChange }: CompletitionBlockProps) => {
    const [modalOpen, setModelOpen] = useState(false);

    return (
        <>
            <Stack>
                <Typography fontWeight={700}>Completition method</Typography>
            ...
            <Modal open={modalOpen} onClose={() => setModelOpen(false)}>
                <Stack gap={2} bgcolor={"#fff"}>
                    ...
                    <Stack direction={'row'} justifyContent={'space-around'}>
                        <Button variant="contained" onClick={() => setModelOpen(false)}>Cancel</Button>
                        <Button variant="contained" onClick={() => { onChange?.(); setModelOpen(false) }}>Clear</Button>
                    </Stack>
                </Stack>
            </Modal>
        </>
