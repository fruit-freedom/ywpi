...
class ExecutionManagerDTO(pydantic.BaseModel):
    ...
    agent: str
    method: str
    last_uid: t.Optional[PyObjectId] = None
                ...
                while True:
                    try:
                        ...
                        if len(items):
                            obj = items[0]
                            if obj['tp'] == 'pdf':
                                print(f'Call {self._agent}/{self._method}')
                                ...
                                print(f'Call {self._agent}/{self._method} end')
                            ...
                            sleep_time = min(1.0 + sleep_time * 2, 10.0)
                    except:
                        import traceback
                        traceback.print_exc()
                        sleep_time = 5.0
...
@router.get('/ems')
async def get_em_list() -> list[ExecutionManagerDTO]:
    return await execution_managers_collection.find().to_list(None)
